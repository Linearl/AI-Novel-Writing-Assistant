...
import type { RepairOptions } from "../../../services/novel/novelCoreShared/index"
import { createChapterDraftExecutableModule } from "./directorChapterDraftStepModule";
...
function createFactOnlyExecutionModule(input: {
  ...
  return createWorkflowStepModule(
    input.descriptor,
    async (moduleInput: { taskId: string; novelId: string }, context): Promise<unknown> => {
...
// ─── Exported step modules ─────────────────────────────────────────────────

const CONTRACT_SYNC_DESCRIPTOR = {
  ...createWorkflowStepDescriptorFromCatalogEntry({
...
export const DIRECTOR_EXECUTION_CONTRACT_SYNC_STEP_MODULE = createWorkflowStepModule(
  CONTRACT_SYNC_DESCRIPTOR,
  async (input: { novelId: string }) => getDirectorCoreStepRuntime().executeChapterExecutionContractSyncStep(input),
  {
    inspectReadiness: async (context) => {
    ...
    inspectCompletion: async (context) => {
      ...
      return plannedChapterCount > 0 && syncedChapterCount >= plannedChapterCount
        ? completedFact(DIRECTOR_EXECUTION_CONTRACT_SYNC_STEP_ID, { evidence: { plannedChapterCount, syncedChapterCount, unsyncedChapterCount } })
        : pendingFact(DIRECTOR_EXECUTION_CONTRACT_SYNC_STEP_ID, {
    ...
    commit: async (_output, context) => {
      ...
      await getDirectorCoreStateCommitter().recordArtifactsIndexed({
        ...
        novelId,
        runtimeId: state.runtime?.id ?? null,
        nodeKey: CONTRACT_SYNC_DESCRIPTOR.nodeKey,
