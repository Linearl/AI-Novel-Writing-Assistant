/**
 * ProseQualityDetector — 纯正则驱动的散文质量检测器
 *
 * 9 种问题码，零 LLM 成本。嵌入章节生成管道末尾，第一时间发现
 * AI 味句式、工程术语泄漏、占位符残留、生成截断等问题。
 *
 * REQ-7072
 */
import type { RuntimeAuditIssue, RuntimeAuditReport } from "@ai-novel/shared";
...
/** P1: AI 式否定翻转句 */
const NEGATIVE_FLIP_PATTERN =
  /(?:不是|并非|并不是|不算|不能说是|没有|不再是)[^。！？；;\n]{1,36}?[，,、]?\s*(?:而是|却是|反而是|更像是|只是)[^。！？；;\n]{1,36}/gu;
...
const AI_SELF_REFERENCE_PATTERN =
  /作为(?:一名|一个)?(?:AI|人工智能|语言模型)|我是(?:AI|人工智能|语言模型)|我无法(?:继续)?(?:创作|生成|提供|完成)|我不能(?:继续)?(?:创作|生成|提供|完成)|无法满足(?:该|这个)?请求|不能协助|as an AI|I (?:am|cannot|can't)[^。！？.!?\n]{0,40}AI/iu;

/** P8: 占位符/TODO/TBD 残留 */
const PLACEHOLDER_PATTERN =
  /TODO|TBD|待补充|此处省略|省略若干|略写|占位|PLACEHOLDER|\{\{[^}]{0,80}\}\}|\[[^\]]{0,40}待补[^\]]{0,40}\]/iu;

/** P9 strong: 强工程术语 */
const ENGINEERING_TERM_STRONG_PATTERN =
  /细纲|情节点|卷纲|功能标签|目标情绪|字数目标|章首钩子|章尾钩子|任务描述|任务单|scene\s*card|prompt|schema|runtime\s*package|上下文包|系统提示词|修复指令/iu;

/** P9 soft: 元叙事/创作说明词 */
const ENGINEERING_TERM_SOFT_PATTERN =
  /本章|下一章|读者|伏笔|前文|后文|剧情推进|人物弧光|爽点|节奏点|钩子/u;

// ─── 主入口 ───────────────────────────────────────────────────────────────

/**
 * 检测章节正文中的散文质量问题。
 * 返回所有发现的问题，每种码 ≤8 条，总计 ≤40 条。
 */
export function detectProseQuality(content: string): ProseQualityReport {
  ...
  const addFinding = (finding: ProseQualityFinding): void => {
    ...
    const duplicated = findings.some(
      (existing) =>
...
/**
 * 将 ProseQualityReport 转换为标准 RuntimeAuditReport。
 * 无 findings 时返回 null。
 */
export function buildProseQualityAuditReport(
  ...
  const issues: RuntimeAuditIssue[] = input.report.findings.map((finding, index) => ({
