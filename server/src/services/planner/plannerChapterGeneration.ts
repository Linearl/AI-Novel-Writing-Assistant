...
export async function generateChapterPlan(
  deps: PlannerChapterGenerationDeps,
  novelId: string,
  chapterId: string,
  options: GenerateChapterPlanOptions = {},
) {
  const [novel, chapter, bible, plotBeats, summaries, characters, bookPlan, arcPlans, volumePlans, recentAuditReports, recentDecisions, storyMacroPlanRow, styleEngine, pendingReviewProposalCount] = await Promise.all([
    ...
    deps.getBookPlan(novelId),
    deps.listArcPlans(novelId),
    ...
    deps.resolvePlannerStyleEngineSummary(novelId, chapterId, options.taskStyleProfileId),
...
export async function executeReplan(
  ...
  for (let index = 0; index < affectedChapters.length; index += 1) {
    ...
    const existingPlan = await deps.getChapterPlan(novelId, chapter.id);
    const plan = await generateChapterPlan(deps, novelId, chapter.id, {
      provider: input.provider,
      model: input.model,
      temperature: input.temperature,
      replanContext: {
