...
import { NovelCorePipelineLoopRunner } from "./novelCorePipelineLoop";
...
export class NovelCorePipelineService {
  ...
  private readonly loopRunner: NovelCorePipelineLoopRunner;

  constructor(db: IDatabase = prisma) {
    this.db = db;
    this.loopRunner = new NovelCorePipelineLoopRunner(db);
  }
  ...
  // ---------------------------------------------------------------------------
  // Execution scheduling (delegates to NovelCorePipelineLoopRunner)
  // ---------------------------------------------------------------------------

  async executePipeline(jobId: string, novelId: string, options: PipelineRunOptions): Promise<void> {
    await this.loopRunner.runPipelineLoop(jobId, novelId, options);
  }
  ...
  private schedulePipelineExecution(jobId: string, novelId: string, options: PipelineRunOptions): void {
    ...
    void this.loopRunner.runPipelineLoop(jobId, novelId, options)
      .catch((err) => {
