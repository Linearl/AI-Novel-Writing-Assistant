...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "",
      "【重要度分级】",
      "1. 每个角色必须标注 tier 字段，允许值：lead（主角/核心人物）、major（重要配角）、named（有名有姓的配角）、extra（路人/龙套）。",
      "2. lead 有且仅有 1 个（故事核心主角），major 控制在 2-4 个，其余为 named 或 extra。",
      "3. extra 角色的 personality、background、development、identityLabel、factionLabel、stanceLabel、powerLevel、realm、currentLocation、availability、prohibitions、outerGoal、innerNeed、fear、wound、misbelief、secret、moralLine、firstImpression 字段可以填空字符串（因为龙套不需要详细设定）。",
      "",
      "【命名硬规则】",
      ...
      "如果上下文是都市/现实/言情/文艺题材，角色的职业和身份必须在本书世界的势力、地点和社会规则内自然成立。pressure_source 的压迫方式必须与世界中已有的势力和规则一致，不能凭空引入世界中不存在的组织或场景。候选角色必须能通过现有势力、已有角色的社会关系、或世界中已有的地点自然地进入故事，而不是作为完全无关的外部存在突然出现。",
      "",
      "【表达要求】",
...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "7. 每个角色必须有 tier 字段，允许值：lead（主角）、major（重要配角）、named（有名配角）、extra（龙套）。",
      "8. 每套方案有且仅有 1 个角色的 tier 为 lead（故事核心主角），major 控制在 2-4 个，其余为 named 或 extra。",
      "",
      "重点修复原则：",
      "1. name 不能再出现”某某位、催化剂、威胁源、功能位、关系变量”这类抽象槽位。",
      "2. 如果上下文存在主角当前身份或隐藏身份线索，至少要让主角方案显式承接这些线索。",
      "3. 同一方案内避免多人承担同一故事功能。",
      "4. 主角（castRole=protagonist）的 tier 必须设为 lead。如果已有主角但 tier 不是 lead，修正为 lead。如果没有角色 tier 为 lead，将主角的 tier 设为 lead。",
    ].join("\n")),
    new HumanMessage([
      "下面这份 JSON 需要修复。",
      "",
...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "6. 必须保持同一故事方向、主角锚点、核心冲突和隐藏身份线索，不要重写成另一套书。",
      "7. 每个角色必须有 tier 字段，允许值：lead（主角）、major（重要配角）、named（有名配角）、extra（龙套）。",
      "8. 有且仅有 1 个角色的 tier 为 lead（故事核心主角），major 控制在 2-4 个，其余为 named 或 extra。",
      "",
      "重点修复原则：",
      "1. name 不能再出现”某某位、催化剂、威胁源、功能位、关系变量”这类抽象槽位。",
      "2. 如果上下文存在主角当前身份或隐藏身份线索，至少要让主角方案显式承接这些线索。",
...
> = {
  ...
  render: (_input, context) => [
    new SystemMessage([
      ...
      "",
      "【重要度分级】",
      "1. 每个候选角色必须标注 tier 字段，允许值：lead、major、named、extra。",
      "2. 注意：已有主角(lead)不要重复分配 lead，补充角色通常是 major 或 named。",
      "3. extra 角色的深层心理和详细背景字段可以填空字符串。",
      "",
      "题材约束（最高优先级）：",
      "1. 候选角色必须严格贴合【分层上下文】中的题材和世界设定，不得生成与题材无关的角色类型。",
      "2. 候选角色的职业、身份、行为必须在本书世界的势力、地点和社会规则内成立——如果世界没有某类组织或场景，就不要生成依赖该设定的角色。",
      "3. pressure_source（压力源）的压迫方式必须与现有剧情和世界规则一致：如果世界是都市情感向，压迫来自生活压力、经济困境、情感纠葛、社会偏见；如果世界有犯罪组织，压迫可以来自犯罪威胁。关键是：压迫来源必须是世界中已有的势力或规则，不能凭空引入。",
      "4. 候选角色必须能自然地进入已有角色的关系网络——通过现有势力、已有角色的社会关系、或世界中已有的地点和场景引入，而不是作为完全无关的外部存在突然出现。",
      "",
      "补位要求：",
