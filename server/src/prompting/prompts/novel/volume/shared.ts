...
export function buildCommonNovelContext(novel: VolumeGenerationNovel): string {
  ...
  return [
    ...
    `描述: ${compactText(novel.description)}`,
    `目标读者: ${compactText(novel.targetAudience)}`,
    `核心卖点: ${compactText(novel.bookSellingPoint)}`,
    `前30章承诺: ${compactText(novel.first30ChapterPromise)}`,
    `叙事视角: ${compactText(novel.narrativePov, "未设置")}`,
    `节奏偏好: ${compactText(novel.pacePreference, "未设置")}`,
