/**
 * World prompts — generation/structure prompts (layer generation, layer localization,
 * import extraction, structure backfill, theme generation, section, axiom suggestion).
...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "3a. 如果主类型是都市/现实/文艺/现代，'力量有代价'应理解为'选择有后果、情感有代价、社会行为有规则'，不能生成超自然法则、魔法体系或奇幻公理。rules.axioms 必须基于现实社会规则（法律、经济、道德、人际关系、职业规则等）。",
      "3b. 现实向题材下：magicSystem 留空，races 留空数组，locations.type 使用现实类型（城市/街区/店铺/医院/学校/公寓等），forces 应为公司/组织/机构/家庭/社团等现实实体。",
      "3c. 现实向公理示例：'情感创伤导致人格封闭，封闭越久重新打开心扉的代价越高'、'大公司可通过合同合法剥夺独立创作者版权'、'渐进性疾病不可逆转，患者必须在有限时间内做选择'。禁止出现'某种力量有副作用'、'使用某种能量消耗生命'等奇幻模式。",
...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "3a. 如果世界类型是都市/现实/文艺/现代，公理必须基于现实社会规则，不能生成超自然法则。现实向公理示例：'情感创伤会导致人格封闭，封闭越久代价越高'、'大公司可以通过合同合法剥夺独立创作者的版权'、'渐进性疾病不可逆转，患者必须在有限时间内做选择'、'聋哑者通过触觉感知振动，但深度共振会造成身体损耗'、'社会偏见会限制特定群体的表达渠道和生存空间'。",
      "3b. 现实向题材禁止出现的公理模式：'某种力量有代价'、'某种能力有副作用'、'使用某种能量会消耗生命'、'违反某种法则会受到惩罚'等奇幻/超自然约束格式。",
