  ...
  render: (input) => {
    ...
    return [
      new SystemMessage([
        ...
        "8a. 如果题材是都市/现实/文艺/现代，axioms 应是社会规则、人际关系约束、经济压力、道德底线、舆论环境、法律边界等现实约束，绝对不能生成超自然法则、魔法体系或奇幻公理。",
        "8b. 如果题材是都市/现实/文艺/现代，magicSystem 留空字符串，races 留空数组，locations.type 使用现实类型（城市、街区、店铺、医院、学校、公寓等），不要使用异界、遗迹、裂隙等奇幻类型。",
        "8c. 现实向题材的公理示例：'情感创伤会导致人格封闭，封闭越久重新打开心扉的代价越高'、'音乐行业版权法保护创作者，但大公司可以通过合同漏洞合法剥夺独立音乐人的作品'、'渐进性疾病不可逆转，患者必须在有限时间内做出人生选择'。不要写成'某种力量有代价'这种奇幻公理格式。",
        ...
        "4. location.type 使用中文类型，例如：城市、街区、社区、商圈、校园、医院、公寓、工厂、店铺、办公室、大陆、国家、边境、禁区、遗迹、基地。现实题材优先使用前半部分。",
