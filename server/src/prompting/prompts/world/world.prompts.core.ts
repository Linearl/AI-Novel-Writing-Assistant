...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "",
      "每项结构必须严格为：",
      '{"severity":"warn|error","code":"...","message":"中文问题概述","detail":"中文详细说明","targetField":"description|background|geography|cultures|magicSystem|politics|races|religions|technology|conflicts|history|economy|factions","suggestion":"具体的修复建议，说明应该如何修改"}',
      "",
      "全局硬规则：",
      "1. message 和 detail 必须使用简体中文。",
      "2. 只能基于给定世界公理、核心设定、检索补充和参考素材进行判断，不得脑补未提供的设定。",
      "3. 只指出真正的冲突或明显风险，不要泛泛而谈，不要为了凑数量制造问题。",
      "4. 如果证据不足，不要强行判定为问题。",
      "5. 如果提供了参考素材，必须对比检查世界设定与参考素材之间的一致性。",
      "",
      "审校重点：",
      "1. 世界公理与具体设定是否冲突。",
      ...
      "5. 世界设定是否与参考素材存在重大偏差或遗漏（如：参考素材中明确规定的规则、势力、地点在世界设定中不存在或相悖）。",
      "6. 参考素材中的关键约束是否被世界设定满足（如：参考素材要求的禁忌、限制、代价在世界设定中有所体现）。",
      "",
      "severity 规则：",
      "1. error：存在明确冲突、无法同时成立、或会直接破坏世界运行逻辑的问题。",
      "2. warn：当前未必绝对冲突，但存在明显高风险、解释缺口或后续极易写崩的点。",
      "3. 参考素材偏差：如果世界设定与参考素材存在重大偏差（如核心设定缺失、规则相悖），标记为 warn 或 error（取决于偏差的严重程度）。",
      "",
      "字段要求：",
      "1. code：使用简洁稳定、可复用的英文或下划线风格问题编码，不要写成长句。",
      "2. message：一句话点明问题核心，让人一眼看懂哪里冲突。",
      "3. detail：具体说明为什么这是问题，冲突发生在哪两层或哪几个字段之间，或者与参考素材的哪个部分不一致。",
      ...
      "5. suggestion：提供具体的修复建议，说明应该如何修改才能解决这个问题。建议应该是可执行的（如：在forces中添加势力X、修改location Y的描述、删除规则Z）。",
      "",
      "参考素材处理规则：",
      "1. 参考素材可能是世界观设定、故事大纲、角色关系、约束条件等任何形式的文档。",
      "2. 对比检查时，应该识别参考素材中的关键元素（如：势力、地点、规则、禁忌、代价、关系），检查它们是否在世界设定中有所体现。",
      "3. 如果参考素材明确要求某些元素必须存在或必须满足，而世界设定中没有，则标记为问题。",
      "4. 如果参考素材与世界设定存在矛盾（如：参考素材说'无魔法'但世界设定有魔法体系），必须标记为 error。",
      "5. 不要机械地逐字对比，而是理解参考素材的意图和约束，检查世界设定是否在精神上符合。",
      "",
      "质量要求：",
