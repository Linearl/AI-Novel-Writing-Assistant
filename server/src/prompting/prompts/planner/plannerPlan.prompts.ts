...
import type { StoryPlanLevel } from "@ai-novel/shared";
...
function buildPlannerSystemPromptLines(planLevel: StoryPlanLevel, includeScenes: boolean): string[] {
  return [
...
function buildPlannerUserPromptLines(scopeLabel: string, contextText: string, includeScenes: boolean): string[] {
  return [
    scopeLabel,
    "",
    "上下文：",
...
function validatePlannerOutput(
  ...
  if (!includeScenes && (normalized.scenes ?? []).length > 0) {
    throw new Error("Planner output should not include scenes for this plan level.");
  }

  if (includeScenes) {
    for (const scene of normalized.scenes ?? []) {
      if (!scene.title?.trim()) throw new Error("Planner scene is missing title.");
      if (!scene.objective?.trim()) throw new Error("Planner scene is missing objective.");
      if (!scene.conflict?.trim()) throw new Error("Planner scene is missing conflict.");
      if (!scene.reveal?.trim()) throw new Error("Planner scene is missing reveal.");
      if (!scene.emotionBeat?.trim()) throw new Error("Planner scene is missing emotionBeat.");
    }
  }
...
function buildPlannerPlanAsset(input: {
  ...
  return {
    ...
    render: (promptInput, context) => {
      ...
      return [
        new SystemMessage(buildPlannerSystemPromptLines(input.planLevel, input.includeScenes).join("\n")),
        new HumanMessage(buildPlannerUserPromptLines(promptInput.scopeLabel, contextText, input.includeScenes).join("\n")),
      ];
    },
    postValidate: (output) => validatePlannerOutput(output, input.planLevel, input.includeScenes),
  };
}
