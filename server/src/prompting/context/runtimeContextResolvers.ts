...
function buildBookContractBlock(writeContext: ChapterWriteContext): PromptContextBlock {
  ...
  return createContextBlock({
    ...
    content: [
      `书名: ${contract.title}`,
      `类型: ${contract.genre}`,
      `目标读者: ${contract.targetAudience}`,
      `核心卖点: ${contract.sellingPoint}`,
      `前30章承诺: ${contract.first30ChapterPromise}`,
      `叙事视角: ${contract.narrativePov}`,
      `节奏偏好: ${contract.pacePreference}`,
      `情绪强度: ${contract.emotionIntensity}`,
      contract.toneGuardrails.length > 0 ? `基调约束: ${contract.toneGuardrails.join(" | ")}` : "",
      contract.hardConstraints.length > 0 ? `硬约束: ${contract.hardConstraints.join(" | ")}` : "",
    ].filter(Boolean).join("\n"),
  });
}
...
function buildStoryMacroBlock(macro: MacroConstraintContext | null): PromptContextBlock | null {
  ...
  return createContextBlock({
    ...
    priority: 98,
    content: [
      `核心卖点: ${macro.sellingPoint}`,
      `核心冲突: ${macro.coreConflict}`,
      `主线钩子: ${macro.mainHook}`,
      `推进循环: ${macro.progressionLoop}`,
      `成长路径: ${macro.growthPath}`,
      `结局风味: ${macro.endingFlavor}`,
      macro.hardConstraints.length > 0 ? `硬约束: ${macro.hardConstraints.join(" | ")}` : "",
    ].filter(Boolean).join("\n"),
  });
}
