...
- `docs/6.changelog/releases/release-notes.md` — 已追加 07-16 变更记录（v0.1 定稿发布 + v0.2 流程自动化落地）
...
本周的核心交付是 v0.1 正式定稿和 v0.2 四大自动化模块的全面落地，以及后续工作范围的上游分析拆分：

- **v0.1 定稿发布**：110 个任务包全部交付，用户可以独立完成小说创作全流程——从项目创建、角色准备、卷章规划到正文生成和审校修复。项目文档（README、Release Notes）也重写为面向新用户的清晰定位和功能说明。

- **自动导演更可靠**：API 失败会自动用指数退避重试，主模型不行会自动切备用模型，断网了会暂停任务保存检查点，恢复网络后继续执行。用户不再需要盯着进度条手动处理点击重试——系统在后台自己处理后通知用户最终结果。

- **长篇小说创作不再阻塞**：300 章任务会自动分解为 30 个批次排队执行，每 10 章保存一个检查点。浏览器关闭、网络断开后重新打开页面会自动从断点继续，不丢进度也不重复已完成的工作。实时进度会显示当前章节、预计剩余时间，用户心里有数。

- **质量自动把关**：每章生成后自动检查 6 个维度（字数、结构、人物、情节、AI 味、一致性），不合格自动重新生成最多 3 次（逐步调参、换模型）。跨章节一致性检查可以识别时间跳跃矛盾、人物复活、空间不一致等逻辑错误。AI 味超过 30% 自动触发反规则重写并可以按章节查看趋势变化。

- **全局审校和角色分级**：跨章节的全局审校可以从 5 个维度发现单章审校遗漏的问题（如人物弧光前后矛盾、伏笔回收遗漏、世界观设定冲突），审校结果自动注入到后续章节的修复流程中。角色按主角/反派/配角三级分级后，生成时主角有最详细的状态追踪和性格一致性约束，背景角色只保留必要信息，避免了关键角色被"稀释"。

- **启动页三种创作路径**：用户可以根据自己的准备程度选择 AI 导演开书（有初步想法）、素材导入（有完善想法和文本材料）、或手动填写（完全自控），三种路径最终汇聚到同一套表单，不失灵活性。

- **写作工作台更流畅**：NovelEdit 主页面从 2731 行拆分到 380 行（缩减 86%），页面加载和 Tab 切换更快速。项目聚焦小说创作方向，移除了漫画/短剧模块，应用更轻量。

- **开发侧进展**：本周还完成了上游功能点的深度分析，将原始的一个大需求包（REQ-7069）拆分为 7 个独立可并行的任务包，明确了创建向导采用"Modal 内嵌可跳过引导式步骤"的整合方案——既保留现有老用户的快捷操作习惯，又为新手提供分步引导。
