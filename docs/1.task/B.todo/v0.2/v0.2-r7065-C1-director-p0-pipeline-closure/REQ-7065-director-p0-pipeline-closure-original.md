...
---
reqId: 7065
title: "导演引擎 P0 Pipeline 闭环收口 — 需求文档（冻结副本）"
...
**目标**：补齐 start/complete/fail/block/cancel 全生命周期的状态写入，确保任意步骤状态变化都有完整状态记录。

**文件**：`server/src/services/novel/director/DirectorStateCommitter.ts`

#### FR-3: Artifact Ledger 接入 Workspace Analyzer

**当前状态**：ArtifactReader / ArtifactWriter 网关已有，但 stale/protected/dependency 影响分析未落地。

**目标**：手动编辑正文或核心设定后，系统能基于 Artifact Ledger 识别 stale/protected 产物，由 Workspace Analyzer 给出受影响范围和最小修复路径。
...
**当前状态**：`novelDirectorProgress.ts` 定义的固定百分比仍在 ~10 个 phase 文件中作为主进度来源（novelDirectorCandidateStage.ts、novelDirectorPipelinePhases.ts、novelDirectorStoryMacroPhase.ts、novelDirectorStructuredOutlinePhase.ts 等）。

**目标**：保留 DIRECTOR_PROGRESS 作为旧 UI 兼容层，StepModule.inspectProgress() 成为主进度来源，Projection 优先消费 inspectProgress 结果。

#### FR-5: Worker 持久化 Delta 写入

**当前状态**：Runtime 持久化每次 mutation 全量重建 steps/events/artifacts，完整 runtime snapshot 仍可能塞回 NovelWorkflowTask.seedPayloadJson。

**目标**：改为增量 delta 写入，projection 查询不读取大体积 workspace/章节正文/prompt context，Worker 长任务运行时 API 仍可响应。
...
- 不改动数据库 schema（不新增 migration）
- 不引入新依赖包
- 所有改动必须通过现有测试套件（directorWorkflowStepModules、directorExecutionService、directorRuntimeExecutionService、directorChapterExecutionProgress、directorControlPlaneBoundary、directorRunCommandService、directorWorker）
...
- [ ] Worker 持久化改为 delta 写入，全量回写不再发生
- [ ] 现有所有导演相关测试通过
- [ ] `pnpm typecheck` 零错误
