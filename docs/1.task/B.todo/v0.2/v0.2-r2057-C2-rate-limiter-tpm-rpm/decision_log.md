---
description: "REQ-2057 决策日志：LLM 限流器 TPM/RPM + 公平调度 + WAL"
reqId: 2057
type: decision_log
created: 2026-07-18
updated: 2026-07-18
...
- **日期**：2026-07-18
- **决策点**：多 novel 并行时的并发槽分配策略
- **选项**：
  - A: 双队列轮转（按来源 round-robin，~80 行）
  - B: 加权公平队列（支持权重配置，~150 行）
  - C: 不做调度（纯 FIFO，依赖 RPM/TPM 兜底）
- **选择**：A（双队列轮转）
- **理由**：
  - 当前没有"优先级"的产品需求，加权队列过度设计
  - 纯 FIFO 在并发槽有限时可能导致一本书饿死另一本
  - 双队列轮转实现简单，保证各 novel 获得约 50% 并发槽
  - 单 novel 时退化为 FIFO，零额外开销
- **风险**：来源数超过 2 时各来源获得的槽位更少，但当前场景最多 2 本并行

## D6: SQLite WAL

- **日期**：2026-07-18
- **决策点**：并行写入时 SQLITE_BUSY 的解决方案
- **选项**：
  - A: 启动时 PRAGMA journal_mode=WAL（1 行代码）
  - B: 实现写入队列串行化（~200 行）
  - C: 迁移 PostgreSQL
- **选择**：A（WAL 模式）
- **理由**：
  - 写入队列收益为零：LLM 调用 10 秒 vs DB 写入 1ms，写入不是瓶颈
  - PostgreSQL 过重，桌面端不适合，当前规模不需要
  - WAL 一行代码解决，并行写入不再互斥，后者等 ~1ms 用户无感
- **风险**：WAL 模式下数据库文件占用略增（wal 文件），可忽略
