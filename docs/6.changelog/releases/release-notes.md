...
| 版本 | 日期 | 里程碑 |
| ------ | ---------- | ------------------------------------------------------------------ |
| v0.1.0 | 2026-03-21 | 项目初始化，基础 CRUD + 导演骨架 |
| v0.1.1 | 2026-03-28 | Schema 补齐与回填 |
| v0.1.2 | 2026-04-06 | 角色、世界观结构与写法引擎 |
| v0.1.3 | 2026-04-13 | PostgreSQL 支持与卷规划 |
| v0.1.4 | 2026-04-20 | 风格提取任务与自动导演跟进 |
| v0.2.0 | 2026-04-27 | 模型路由、Provider 请求限制与角色库同步 |
| v0.2.1 | 2026-05-03 | 自动导演后台运行态与新手引导 |
| v0.2.2 | 2026-05-08 | 提示词工作台、桌面客户端正式发布 |
| v0.3.0 | 2026-05-15 | 短剧创作链路与世界观工作台 |
| v0.3.1 | 2026-05-22 | 漫画分格与短剧质量检查 |
| v0.3.2 | 2026-05-29 | 角色阵容、自动导演整本合并 |
| v0.3.3 | 2026-06-02 | 世界手册、世界骨架流程 |
| v0.3.4 | 2026-06-08 | 质量守卫、事实账本、JIT 规划、生产链路四阶段优化 |
| v0.3.5 | 2026-06-10 | 事实账本验收覆盖、短剧视频生产闭环 |
| v0.3.6 | 2026-06-11 | 章节写作进度、后置提取收敛 |
| v0.3.7 | 2026-06-12 | 版本快照保留与数据库体积治理 |
| v0.3.8 | 2026-06-14 | 设置页创作可用性控制台 |
| v0.3.9 | 2026-06-15 | 漫画角色视觉资产升级 |
| v0.3.10 | 2026-06-16 | 漫画角色视觉识别与分格提示词控制 |
...
| v0.1.1 | 2026-08-11 | 桌面端启动稳定性修复与 CI 发布链路修复 |
| v0.1.2 | 2026-08-12 | 修复 GitHub 下载版启动崩溃（种子数据库随包内置） |
| v0.1.3 | 2026-08-12 | 修复 GitHub 下载版启动崩溃（API Token 写入可写目录） |
| v0.1.4 | 2026-08-12 | 修复自动更新不可用与迁移链历史缺陷 |
| v0.1.5 | 2026-08-12 | 修复桌面版界面接口报错（API 鉴权适配桌面模式） |
| v0.1.6 | 2026-08-12 | 修复设置页接口报错（数据库结构与模型对齐） |
| v0.1.7 | 2026-08-12 | 构建链路修复（本地/发布产物一致性） |
| v0.1.8 | 2026-08-12 | 种子数据库确定性生成（支持产物逐字节校验） |

## 更新历史

### 2026-08-12（v0.1.8 发布）

进一步修复构建链路，支持本地构建与官方发布产物的逐字节校验：

- 种子数据库改为确定性生成：预设数据标识与时间戳统一为固定值，同一代码在任何环境构建出完全一致的种子数据库。
- 构建流程不再重复编译原生模块，避免本地多次构建后开发环境被污染。

### 2026-08-12（v0.1.7 发布）
...
### 2026-08-12（v0.1.3 发布）

修复从 GitHub 下载的桌面版首次启动仍会退出的问题：

- 修复桌面版启动崩溃：API Token 不再写入只读的安装目录，改为保存在用户数据目录，首次启动即可正常完成初始化。

### 2026-08-12（v0.1.2 发布）

修复从 GitHub 下载的桌面版首次启动即退出的问题：

- 修复桌面版启动崩溃：打包产物内置完整种子数据库，新环境首次启动即可正常初始化，不再依赖本机开发数据。
- 修复干净数据库上迁移链执行失败的问题：发布产物按当前数据结构直接建库并标记全部迁移已完成，不再回放历史迁移。
- 增强发布构建链路：CI 无本地开发数据库时自动生成种子数据库，保证每次发布产物完整可用。

修复桌面端启动稳定性与 CI 正式发布链路：
...
v0.1 版本正式定稿，完成了从架构修复到易用性改进的 110 个任务包，核心体验闭环：用户可以独立完成小说创作全流程，AI 味控制在可接受范围内。《玫瑰窃贼》作为验证项目正在推进 18 万字、30 章的完整创作。v0.2 流程自动化进入执行阶段，M1~M4 四个里程碑全部完成（异常自动处理、长篇生成优化、质量自动控制、用户体验优化），共 23 个任务包已归档；上游功能点分析也已完成，识别出 7 个高价值功能需求并拆分为独立任务包。写作工作台和自动导演也更稳定：全局审校可跨章节发现一致性问题和质量问题并回灌到章节修复，角色可以按主角/反派/配角三级重要度分级管理，前端控制面板和资产面板也更有条理。

- v0.1 版本正式定稿并发布完整 Release Notes，记录 110 个任务包按 9 大领域分类的交付成果。
- 项目 README 重写：移除上游截图引用、标注 fork 独有功能、更新项目定位，面向新用户更清晰。
- 全局审校系统上线：覆盖文笔质量、人物一致性、情节逻辑、节奏把控、世界观协调 5 个维度的跨章节审查，审校结果自动注入后续章节修复流程。
- 角色重要度分级（主角/反派/配角/命名角色/背景角色）贯穿全链路：创建时可选 tier、侧边栏按分级筛选、上下文按 tier 详略输出、正文生成/审校/修复均感知角色重要性。
- 批量风格润色支持同时处理多章，后台异步执行、进度实时可见、完成后直接替换章节正文。
- 用户反馈窗口重写：右下角悬浮按钮一键弹出，支持粘贴截图、AI 自动收集运行上下文（<50KB 环形缓冲），生成结构化 GitHub Issue 并提供 Markdown 预览和复制功能。
- 审校上下文增强：light/full audit prompt 新增 5 组 context 要求，review 模式强制注入角色时间线、人物动态关系、伏笔回收指令。
- v0.2 M1 异常自动处理全部完成：API 失败指数退避自动重试（1s→2s→4s，最多 3 次）、429 限流自动解析 Retry-After、主模型失败自动切换备用模型且不中断生成、4 类错误自动分类（可恢复/需配置/需人工/系统错误）、后台心跳网络监控断网时自动暂停任务并保存检查点。
- v0.2 M2 长篇生成优化全部完成：每 10 章自动保存完整状态检查点、检查点自动清理保留最近 5 个、断点续传从检查点恢复不重复已完成工作、300 章自动分解 30 批次排队执行、WebSocket 实时推送进度含 ETA 计算。
- v0.2 M3 质量自动控制全部完成：每章生成后自动 6 维度质量检查（字数/结构/人物/情节/AI味/一致性）、不合格自动重做最多 3 次逐步调整参数、AI 味检测超 30% 自动触发反规则重写、4 类一致性矛盾检测（时间/人物/空间/逻辑）、跨章节人物状态快照结构化存储与变化合理性校验、AI 味趋势追踪含移动平均和波动率可视化。
- v0.2 M4 用户体验优化全部完成：长任务移入后台不阻塞界面、浏览器原生通知关键事件（任务完成/失败/需干预）、服务启动时自动诊断配置问题并给出修复建议。
- 上游移植部分已交付：中文本地化（系统提示词和 UI 文案全面中文化）、LLM 基础设施增强、冲突曲线可视化编辑器（React Flow + d3-shape）、导演引擎 P0 Pipeline 闭环收口、artifactSyncMode 前端选择器、多素材导入系统、导演步骤间推理链路传递。
- 上游功能分析完成：原始 REQ-7069（7 个子功能）经对比分析后拆分为 7 个独立任务包，创建向导采用"Modal 内嵌可跳过引导式步骤"整合方案，保留现有快速模式的同时提供分步引导体验。
- 架构知识库和版本计划文档同步更新至 36 个任务包（23 done + 13 todo），周报新增上游分析拆分详细记录。
- 启动页新增三种创作路径选择卡片，方便用户根据准备程度选择 AI 导演开书、素材导入或手动填写。
