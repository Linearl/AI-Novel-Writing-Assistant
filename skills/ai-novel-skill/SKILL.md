...
## Claude-Native Execution Boundary

**Claude is the only engine for creative understanding, planning, generation, review, and judgment.** This Skill is the process contract; Python scripts perform only deterministic state transitions, validation, indexing, conflict detection, and export. This repository is not a runtime or submodule: do not add provider SDKs, Web APIs, database authorities, queues, or a custom agent runtime. `provider` and `model` fields are Token diagnostics only when the host exposes them, never a generation dependency.

## Check Skill Currency Before Running

Treat the checked-out repository as the editable Skill source. Before the first production action in a task, compare the repository root with the installed Skill directory.
...
In a workspace, use `scripts/novelctl.py` as the deterministic control surface for state transitions, validation, recovery, context, checkpoints, usage, and export. Claude remains the only creative generation engine; do not add or invoke a model-provider SDK from this skill.
