    ...
    <Card className="flex h-full min-h-0 flex-col">
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
        ...
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
          {threads.map((thread) => (
            <div
              ...
              className={`rounded-xl border px-3 py-2.5 ${
                thread.id === activeThreadId ? "border-slate-900 bg-muted" : "border-border bg-white"
              }`}
            >
              <button
                type="button"
              ...
              <div className="mt-2 flex gap-2 text-[11px]">
                <button
                  type="button"
                  className="rounded-md border border-border px-2 py-1 text-slate-600 transition hover:bg-secondary"
                ...
                <button
                  type="button"
                  className="rounded-md border border-red-200 px-2 py-1 text-destructive transition hover:bg-red-50"
          ...
          {threads.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border bg-muted p-4 text-sm text-slate-500">
              还没有创作中枢线程。
            </div>
          ) : null}
        </div>
