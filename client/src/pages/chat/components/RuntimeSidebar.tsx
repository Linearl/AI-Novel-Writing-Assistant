...
export default function RuntimeSidebar({
  ...
  return (
    <Card className="sticky top-4 flex h-[calc(100vh-8rem)] flex-col border-border shadow-sm">
        ...
        <div className="flex rounded-lg bg-secondary p-1">
          <button
            ...
            className={`flex-1 rounded-md px-3 py-2 text-xs font-medium transition ${
              activeTab === "console" ? "bg-white text-foreground shadow-sm" : "text-muted-foreground"
            }`}
          ...
          <button
            type="button"
            className={`flex-1 rounded-md px-3 py-2 text-xs font-medium transition ${
              activeTab === "trace" ? "bg-white text-foreground shadow-sm" : "text-muted-foreground"
            }`}
        ...
        {activeTab === "console" ? (
          <div className="space-y-3">
            <div className="rounded-xl border border-border bg-white p-3">
              <div className="mb-2 text-xs font-medium tracking-wide text-muted-foreground">会话上下文</div>
              <div className="grid gap-2">
                <div className="grid gap-1">
                  <label className="text-[11px] text-muted-foreground">对话模式</label>
                  <select
                ...
                <div className="grid gap-1">
                  <label className="text-[11px] text-muted-foreground">上下文模式</label>
                  <select
                    className="w-full rounded-lg border border-border bg-white p-2"
                    value={contextMode}
                ...
                {runHistoryIds.length > 0 ? (
                  <div className="grid gap-1">
                    <label className="text-[11px] text-muted-foreground">会话运行</label>
                    <select
                      className="w-full rounded-lg border border-border bg-white p-2"
                      value={currentRunId}
                ...
                {contextMode === "novel" ? (
                  <div className="grid gap-1">
                    <label className="text-[11px] text-muted-foreground">小说</label>
                    <select
                      className="w-full rounded-lg border border-border bg-white p-2"
                      value={novelId}
            ...
            <div className="rounded-xl border border-border bg-white p-3">
              <div className="mb-2 flex items-center justify-between">
                <div className="text-xs font-medium tracking-wide text-muted-foreground">审批</div>
                ...
                <div className="space-y-3">
                  {approvalCards.map((item, index) => (
                    <div key={item.approvalId} className="rounded-xl border border-amber-200 bg-amber-50/60 p-3">
                      <div className="text-sm font-semibold text-foreground">审批项 {index + 1}</div>
                      <div className="mt-1 text-xs text-muted-foreground">{item.targetType}:{item.targetId}</div>
                      <div className="mt-2 rounded-lg bg-white p-2 text-sm text-foreground">{item.summary}</div>
                    </div>
                  ))}
                  <Textarea
                    className="min-h-[88px] rounded-lg border-border bg-muted"
                    value={approvalNote}
              ...
              {approvalHistory.length > 0 ? (
                <details className="mt-3 rounded-xl border border-border bg-muted p-2">
                  <summary className="cursor-pointer px-1 py-1 text-xs font-medium text-foreground">
                    审批历史
                  </summary>
                  <div className="mt-2 space-y-2">
                    {approvalHistory.map((item) => (
                      <div key={item.id} className="rounded-lg border border-border bg-white px-2 py-2 text-xs">
                        <div className="font-medium text-foreground">{item.status} · {item.targetType}:{item.targetId}</div>
                        {item.decisionNote ? <div className="mt-1 text-muted-foreground">{item.decisionNote}</div> : null}
                      </div>
                    ))}
                  </div>
                </details>
            ...
            <details className="rounded-xl border border-border bg-white">
              ...
              <div className="space-y-3 border-t border-border p-3">
                <div>
                  <div className="mb-2 text-xs font-medium tracking-wide text-muted-foreground">模型</div>
                ...
                <div>
                  <div className="mb-2 text-xs font-medium tracking-wide text-muted-foreground">系统提示词</div>
                  <Textarea
                    className="min-h-[110px] rounded-lg border-border"
                    value={systemPrompt}
          ...
          <div className="space-y-3">
            {replayableSteps.length > 0 ? (
              <div className="rounded-xl border border-border bg-muted p-3">
                <div className="text-xs font-medium text-muted-foreground">重放控制</div>
                  ...
                  <details key={item.key} className={`rounded-xl border px-3 py-2 text-xs ${stepStatusTone(item.step.status)}`}>
                    <summary className="cursor-pointer list-none">
                      <div className="flex items-start gap-3">
                        <div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white text-[11px] font-semibold text-foreground ring-1 ring-border">
                          {item.step.seq}
                        </div>
                    ...
                    <div className="mt-2 space-y-2">
                      ...
                      {item.step.error ? <div className="text-destructive">错误: {item.step.error}</div> : null}
                    </div>
                  </details>
                ) : (
                  <div key={item.key} className="rounded-xl border border-border bg-muted px-3 py-2 text-xs">
                    <div className="mb-1 text-[11px] text-muted-foreground">事件 {index + 1}</div>
