...
Token budget 分配（320K 上限）：

| 组件 | 每项 tokens | 数量 | 小计 |
|------|------------|------|------|
| 系统 prompt + 输出格式 | 2K | 1 | 2K |
| **全局层** | | | **~10K** |
| ├ book_contract | 500 | 1 | 500 |
| ├ story_macro | 500 | 1 | 500 |
| ├ 角色弧线规划 | 3K | 1 | 3K |
| ├ 伏笔总账 | 3K | 1 | 3K |
| └ 当前卷概览 | 1K | 1 | 1K |
| **章节层（每章）** | | | |
| ├ 结构化摘要 | ~1.5K | N | 1.5N |
| └ 全文 | ~8.5K (6000字÷1.5) | N | 8.5N |
| 预留输出 | 10K | 1 | 10K |

**预算测算**（40章 × 6000字）：
- 全文 + 摘要 = (8.5K + 1.5K) × 40 = 400K → 超出 320K 约 25%
- 自动裁剪策略：320K - 12K(全局+系统+输出) = 308K → 308K / 10K ≈ **30章**
- 超出时优先丢弃远处章节的全文（只保留摘要），确保近距离章节有全文

**结论**：320K 预算下，一卷 40 章无法全部审完，自动裁剪到 ~30 章。用户选"全卷"时提示"本次覆盖第 X-Y 章（30/40），剩余章节需单独审"。
