...
| # | 任务 | 优先级 | 预估 | 状态 |
| --- | --- | --- | --- | --- |
| T1 | 后端：定义阶段枚举与 SSE 进度事件协议（shared 类型） | P0 | 0.5h | ⬜ 待开始 |
| T2 | 后端：拆分骨架生成为多阶段流式执行器 | P0 | 2h | ⬜ 待开始 |
| T3 | 后端：新增 SSE 流式骨架生成路由 | P0 | 1h | ⬜ 待开始 |
| T4 | 前端：消费 SSE 进度事件并更新按钮文案 | P1 | 1.5h | ⬜ 待开始 |
| T5 | 端到端验证与回归测试 | P1 | 0.5h | ⬜ 待开始 |
