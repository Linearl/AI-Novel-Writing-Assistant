...
- **文件**: `server/src/modules/novel/http/novelRouteRegistration.ts` 或相关路由文件
- **工作内容**:
  - 新增 `GET /tasks/:taskId/versions` 接口
  - 查询修复过程中的版本记录
- **DoD**: API 返回版本列表
- **估时**: 0.5h
- **依赖**: 无
...
### T-2.1: Token API 函数
- **文件**: `client/src/api/novel/chapters.ts` 或新建 `client/src/api/novel/repair.ts`
- **工作内容**:
  - 新增 `getRepairTokenUsage(taskId)` 函数
- **DoD**: 类型检查通过
- **估时**: 0.25h
- **依赖**: T-1.1

### T-2.2: Token 实时显示
- **文件**: `client/src/pages/novels/components/RepairDetailDialog.tsx`（需新建或定位现有组件）
- **工作内容**:
...
### T-3.1: 版本列表 API 函数
- **文件**: `client/src/api/novel/repair.ts`
- **工作内容**:
  - 新增 `getRepairVersions(taskId)` 函数
- **DoD**: 类型检查通过
- **估时**: 0.25h
- **依赖**: T-1.2
