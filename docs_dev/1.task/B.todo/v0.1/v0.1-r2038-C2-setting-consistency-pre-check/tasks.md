...
| 三 | T6 | FE-001 | 设定页面校验结果展示 | P2 | 3h | done |
| 三 | T7 | FE-002 | 一键修复功能 | P2 | 3h | done |
| 三 | T8 | FE-003 | 忽略功能 | P2 | 1h | done |
| 三 | T9 | VER-001 | 全量验证 | P2 | 2h | done |
...
**子任务**:
- [x] T6.1 创建 `SettingConsistencyPanel.tsx` 组件
- [x] T6.2 展示总体评分（pass / warning / fail）+ 校验时间
- [x] T6.3 展示矛盾列表（字段、描述、严重级别、修复建议）
- [x] T6.4 添加"校验一致性"触发按钮
- [x] T6.5 加载状态和空状态处理
...
**子任务**:
- [x] T7.1 在矛盾项上添加"一键修复"按钮
- [x] T7.2 调用修复 API，展示修复进度
- [x] T7.3 修复完成后刷新校验结果
- [x] T7.4 修复失败时展示错误信息

**验收**: 一键修复正常工作

> 注: 修复 API 端点推迟到后续迭代；当前 T7.1 为占位实现，展示修复建议文案。
...
**子任务**:
- [x] T8.1 在矛盾项上添加"忽略"按钮
- [x] T8.2 调用忽略 API
- [x] T8.3 忽略后该项从列表消失
- [x] T8.4 添加"查看已忽略"入口（可选）
