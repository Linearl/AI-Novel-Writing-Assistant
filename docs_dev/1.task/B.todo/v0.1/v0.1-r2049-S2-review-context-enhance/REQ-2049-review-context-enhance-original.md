...
| `payoff_directives` | 98 | `includePayoffDirectives = writeContext.payoffDirectives.length > 0` — review 上下文可能为空 |
| `recent_chapters` | 86 | `includeRecentChapters = mode === "full"` — review 模式为 `"review"`，永远不注入 |

审校 AI 不知道书的核心定位（类型、读者、卖点、POV、节奏），不知道故事宏观结构（核心冲突、主钩子、成长路径），不知道时间线约束，不知道角色关系动态，不知道伏笔操作指令，最近章节也只有极有限的摘要。
...
### 3.2 功能 2：给审校注入 timeline_context、character_dynamics、payoff_directives

**目标文件**：`server/src/services/audit/auditPromptContext.ts`

当前 `resolveAuditChapterContextBlocks` 调用 `buildChapterReviewContextBlocks(reviewContext)` 来获取 fallback context blocks。该函数已包含 `timeline_context`（条件注入）和 `character_dynamics`（条件注入），但条件可能阻止注入。`payoff_directives` 在 review 模式下也可能被跳过。
...
- `payoff_directives` 在 review 模式下即使 `writeContext.payoffDirectives` 为空也注入（输出"无待操作伏笔"占位）
...
- `includePayoffDirectives` 始终为 `true`（即使无数据，也输出空状态提示）
