...
| # | 任务 | 优先级 | 预估 | 状态 |
| --- | --- | --- | --- | --- |
| T1 | shared/types：Chapter 类型增加 locked 字段 | P0 | 0.5h | ⬜ 待开始 |
| T2 | Prisma schema：Chapter 模型增加 locked Boolean 字段 | P0 | 0.5h | ⬜ 待开始 |
| T3 | 数据库迁移：执行 prisma migrate dev | P0 | 0.5h | ⬜ 待开始 |

### 阶段二：server 层

| # | 任务 | 优先级 | 预估 | 状态 |
| --- | --- | --- | --- | --- |
| T4 | API 端点：PATCH /chapters/:id/lock 切换锁定状态 | P0 | 1h | ⬜ 待开始 |
| T5 | auto-director replan：过滤 locked 章节 | P0 | 1.5h | ⬜ 待开始 |
| T6 | auto-director full_audit：过滤 locked 章节 | P0 | 1h | ⬜ 待开始 |
| T7 | auto-director 补充关系网：过滤 locked 章节 | P1 | 1h | ⬜ 待开始 |
| T8 | auto-director 补充时间线：过滤 locked 章节 | P1 | 1h | ⬜ 待开始 |
| T9 | auto-director 章节标题修复：过滤 locked 章节 | P1 | 0.5h | ⬜ 待开始 |

### 阶段三：client 层

| # | 任务 | 优先级 | 预估 | 状态 |
| --- | --- | --- | --- | --- |
| T10 | 章节列表：锁定按钮组件 | P0 | 1.5h | ⬜ 待开始 |
| T11 | 章节列表：锁定状态视觉标识 | P0 | 1h | ⬜ 待开始 |
| T12 | 集成测试：锁定/解锁端到端流程 | P1 | 1h | ⬜ 待开始 |
...
**DoD**:
- [ ] Chapter 类型包含 `locked: boolean` 字段
- [ ] 构建 shared 通过（`pnpm --filter @ai-novel/shared build`）
...
- [ ] 迁移文件包含 ALTER TABLE 添加 locked 列
...
**DoD**:
- [ ] PATCH /api/chapters/:id/lock 可用
- [ ] 请求体支持 `{ locked: boolean }`
- [ ] 返回更新后的章节数据
- [ ] chapterId 不存在时返回 404
...
**DoD**:
- [ ] replan 不修改 locked 章节的任何字段
- [ ] 非 locked 章节的 replan 行为不受影响
...
**DoD**:
- [ ] full_audit 不对 locked 章节生成审查意见
- [ ] 非 locked 章节的审查行为不受影响
...
**DoD**:
- [ ] 未锁定章节显示解锁图标，点击后切换为锁定状态
- [ ] 已锁定章节显示锁定图标，点击后切换为解锁状态
- [ ] 调用 PATCH /api/chapters/:id/lock 更新状态
- [ ] 操作后章节列表即时更新
...
**DoD**:
- [ ] 已锁定章节显示锁图标
- [ ] 已锁定章节的视觉样式与未锁定章节有明显区分（如置灰、边框色变化等）
- [ ] hover 时显示 tooltip 说明锁定状态
...
**DoD**:
- [ ] 锁定章节后，replan 不修改该章节
- [ ] 解锁章节后，replan 恢复修改该章节
- [ ] 所有章节锁定时，replan 正常跳过不报错
